...
export const Table = <T extends {}>({ id, data, columns }: TableProps<T>) => {
  ...
  return (
    <div id={id} className="overflow-x-auto border border-gray-200 rounded-xl shadow-sm flex-1 min-h-0">
          ...
          {displayData.map((item, index) => (
            <tr key={index} className="hover:bg-gray-50">
              {columns.map((column) => {
                ...
                return (
                  <td key={`${index}-${column.key as string}`} className="px-6 py-4 whitespace-nowrap text-sm text-gray-800">
                    {column.renderer ? <column.renderer value={value} item={item} /> : String(value)}
                  </td>
                );
              })}
            </tr>
