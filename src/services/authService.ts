...
export const newRegisterUser = async (formData: RegistrationFormData) => {
  try {
    // Validate email format
...
export const registerUserAfterVerification = async (
  ...
  try {
    ...
    // Create stripe user
    const responseCustomerStripe = await createStripeUser(
      formData.correo,
      authData.user.id
    );
    ...
    const responseViajero = await createNewViajero(formData, [
      responseCompany.empresa_id,
    ]);
